    ...
    def adv_attack(self, model, **inputs):
        ...
        if self.random_init or self.steps == 0:  # steps == 0 means that it'll just be a random latent perturbation
            ...
            if len(current_perturbation.shape) == 4:  # Q, K, or V perturbation
                n = d_flat.norm(p=2, dim=1).view(batch_size, 1, 1, 1) + 1e-6
            else: # residual perturbation
                n = d_flat.norm(p=2, dim=1).view(batch_size, 1, 1) + 1e-6
        ...
        # not this part problem
        # # tensor([[[ 0.0025,  0.0105, -0.0066,  ...,  0.0088,  0.0186, -0.0073],
        #  [ 0.0089,  0.0017, -0.0122,  ..., -0.0058,  0.0092,  0.0143],
        #  [ 0.0142, -0.0065,  0.0008,  ..., -0.0041,  0.0042,  0.0008],
        #  ...,
        #  [-0.0009, -0.0258,  0.0072,  ...,  0.0028, -0.0021, -0.0134],
        #  [ 0.0055, -0.0043, -0.0123,  ...,  0.0036, -0.0062, -0.0149],
        #  [-0.0036, -0.0067, -0.0037,  ...,  0.0075, -0.0098,  0.0029]]],
        ...
        for step in range(self.steps):
            model.zero_grad()
            contains_nan = torch.isnan(current_perturbation  ).any()
            # if contains_nan:
            #     print( step, "1 current_perturbation 张量中包含NaN值")
            # else:
            #     print( step, "1 current_perturbation  张量中不包含NaN值")
            # Get loss
            ...
            if labels is not None:
                ...
                contains_nan = torch.isnan(loss ).any()
                # if contains_nan:
                #     print("loss张量中包含NaN值")
                # else:
                #     print("loss张量中不包含NaN值")
            ...
            contains_nan = torch.isnan(grad  ).any()
            if contains_nan:
                pass
                #print( step, "grad 张量中包含NaN值")
            else:
                #print( step, "grad 张量中不包含NaN值")
            # print("Step:", step, "New Grad norm:", grad.norm().item())
            # modify grad as needed
                ...
                # contains_nan = torch.isnan(current_perturbation  ).any()
                # if contains_nan:
                #     print( step, "1.5 current_perturbation 张量中包含NaN值")
                # else:
                #     print( step, "1.5 current_perturbation  张量中不包含NaN值")
                # print("Step:", step, "Grad", grad.item())
                #print("Step:", step, "Grad norm:", grad.norm().item())
                # update perturbation
                current_perturbation = current_perturbation.detach() + stepsize * grad
            # contains_nan = torch.isnan(current_perturbation  ).any()
            # if contains_nan:
            #     print( step, "2 current_perturbation 张量中包含NaN值")
            # else:
            #     print( step, "2 current_perturbation  张量中不包含NaN值")
            # project, scale clip
            current_perturbation = project_scale_clip(current_perturbation)
            #print("Step:", step, "Grad norm:", current_perturbation.norm().item())
            # contains_nan = torch.isnan(current_perturbation  ).any()
            # if contains_nan:
            #     print( step, "3 current_perturbation 张量中包含NaN值")
            # else:
            #     print( step, "3 current_perturbation  张量中不包含NaN值")
        model.zero_grad()
