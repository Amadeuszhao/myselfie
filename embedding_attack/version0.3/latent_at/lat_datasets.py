...
class LatentAdversarialTrainingDataCollator:
    ...
    def __call__(self, batch):
        ...
        for i in range(B):
            adv_padd_prompt_tokens[i] = torch.tensor(batch[i]["prompt_tokens"][:-6] +[40] * 20 + batch[i]["prompt_tokens"][-6:] + batch[i]["adv_tokens"] + [self.pad_token_id] * (pad_length - adv_prompt_lengths[i] ))
            def_padd_prompt_tokens[i] = torch.tensor(batch[i]["prompt_tokens"][:-6] +[40] * 20 + batch[i]["prompt_tokens"][-6:] + batch[i]["def_tokens"] + [self.pad_token_id] * (pad_length - def_prompt_lengths[i] ))
