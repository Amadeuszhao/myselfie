...
def projected_gradient_descent_onpadd(
    ...
    for j in range(pgd_iterations):
        ...
        for adv in adversaries:
            embedding_loss = adv.calculate_top_k_embedding_loss(k=20)
            embedding_loss = embedding_loss   # 调整嵌入约束的权重
        ...
        if (j > 21 and j % 5==1):
            if best_loss <= losses['adv_total']:
                ...
                print("update new best loss")
                best_loss = losses['adv_total']
                adv.save_uap("/root/autodl-tmp/at/best_uap/best")
        ...
        adv_optim.step()
        if (j >=20 and j % 5==0) or j == pgd_iterations -1:
            for adv in adversaries:
                adv.clip_attack()
