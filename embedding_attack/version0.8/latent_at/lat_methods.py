...
def projected_gradient_descent_onpadd(
        logger,
        tokenizer: None,
    ...
    for j in range(pgd_iterations):
        ...
        for adv in adversaries:
            ...
            embedding_loss = embedding_loss * 500   # 调整嵌入约束的权重
            embedding_loss.backward()
            # embedding_loss_all = adv.calculate_embedding_loss()
            # embedding_loss_all = embedding_loss_all * 10
            # embedding_loss_all.backward()
        ...
        if embedding_constraint:
            if embedding_method == 'model':
                embedding_dim = 4096
                classifier = MLPClassifier(embedding_dim).to(dtype=torch.float16).cuda()
                classifier = classifier.cuda()
                classifier.load_state_dict(torch.load('/root/autodl-tmp/myselfie/class.pth'))
                loss_embed_constraint = 0.0

                for adv in adversaries:
                    #print(adv.attack[adv.attack_mask[:, :]].shape)
                    output_prob = classifier(adv.attack[adv.attack_mask[:, :]]).squeeze()
                    # 计算损失（希望分类器输出接近1，即属于嵌入空间）
                    criterion_embed_constraint = nn.BCELoss()
                    target_embed_constraint = torch.ones_like(output_prob)
                    loss_embed_constraint += criterion_embed_constraint(output_prob, target_embed_constraint)
                #print("Compute constraint")
                loss_embed_constraint = loss_embed_constraint 
                # 反向传播
                loss_embed_constraint.backward()
            elif embedding_method == 'compare':
                embeddings = np.load("/root/autodl-tmp/myselfie/embeddings.npy")
        if (j > 21) and j % 10 == 1:
            if best_loss <= losses['adv_total']:
                pass
            else:
                
                best_loss = losses['adv_total']
                print("update new best loss", best_loss)
                # adv.save_uap(best_path)
        ...
        if j >=20 and j % 10 == 0: 
            ...
            with torch.no_grad():
                ...
                print(batch["adv_padd_prompt_tokens"][0][0:batch["prompt_length"][0][2]].unsqueeze(0).to("cuda"))
                shape_1 = batch["adv_padd_prompt_tokens"][0][0:batch["prompt_length"][0][2]].unsqueeze(0).to("cuda").shape[1]
    ...
    if  j == pgd_iterations -1:
        ...
        # if j % 200 == 0:
        #     #print(adv.attack[adv.attack_mask[:, :]].shape)
        #     current_folder = create_iteration_folder()
        #     save_tensor_as_numpy(adv.attack[adv.attack_mask[:, :]].detach().cpu(), current_folder, j)
    if return_loss_over_time:
