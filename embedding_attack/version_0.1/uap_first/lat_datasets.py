...
class LatentAdversarialTrainingDataCollator:
    ...
    def __call__(self, batch):
        ...
        for i in range(B):
            adv_padd_prompt_tokens[i] = torch.tensor(batch[i]["prompt_tokens"][:-6] +[self.pad_token_id] * 20 + batch[i]["prompt_tokens"][-6:] + batch[i]["adv_tokens"] + [self.pad_token_id] * (pad_length - adv_prompt_lengths[i] ))
            def_padd_prompt_tokens[i] = torch.tensor(batch[i]["prompt_tokens"][:-6] +[self.pad_token_id] * 20 + batch[i]["prompt_tokens"][-6:] + batch[i]["def_tokens"] + [self.pad_token_id] * (pad_length - def_prompt_lengths[i] ))
...
def process_generic_chat_dataset(
    ...
    **dataset_kwargs,
    ...
    elif dataset == 'LLM-LAT/harmful-dataset':
        print('load harmful')
        
        dataset = load_dataset('parquet', data_files="/root/autodl-tmp/base/harmful.parquet",split='train')
        #print(dataset['train'].to_pandas().head())
       # print(set(dataset.column_names) - {"prompt", "adv_completion", "def_completion"})
