...
#         self.extra_lm_head = nn.Linear(config.hidden_size * 2, config.vocab_size, bias=False)
...
#         combine_hidden_states = torch.cat((hidden_states, target_hidden_states), dim=2)
...
#             logits = self.extra_lm_head( combine_hidden_states)
...
# # %%
# model_id = "/root/autodl-tmp/repeat_output_combine/checkpoint-50000"
...
with open("/root/autodl-tmp/repe/outputs_short.json",'r') as f:
    outputs = json.load(f)
outputs
...
#     save_hidden_states = []
#     for i in range(32):
#         temp = np.load(f"/root/autodl-tmp/repe/short_hidden_states/harmful_{idx}/harmful_hidden_state{i}.npy")
...
#     with open(f"/root/autodl-tmp/myselfie/short_results_original/{idx}.json",'w') as f:
...
for idx in range(405):
    ...
    for layer in range(31):
        ...
        output = hidden_states_decoder(mt.model, save_hidden_states[layer],  layers, token, attn)
        outputs.append(output)
    with open(f"/root/autodl-tmp/myselfie/short_results_original/layer_{idx}.json",'w') as f:
        json.dump(outputs,f,indent =4)
